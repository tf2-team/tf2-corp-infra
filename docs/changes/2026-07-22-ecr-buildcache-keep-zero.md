# Change: ECR lifecycle keep zero buildcache images

## Summary

ECR lifecycle policy now keeps **0** `:buildcache`-tagged images (expire after 1 day) and retains a maximum of **5** other images per service repository in both development and production.

## Context

Registry build-cache tags (`:buildcache`) grow storage and compete with deployable image retention. Operators requested buildcache retention of **0** while keeping runtime (and other) images capped at **5**.
...
* Lifecycle rule 2: `tagStatus = any`, keep last **5** images.
...
* Rule 2 still keeps last **5** images (any tag).
...
4. Updated env variable descriptions/defaults and `docs/COST.md` keep counts.
...
**Environments:**
* `environments/development/terraform.tfvars` — `ecr_keep_last_n_buildcache = 0`.
* `environments/development/variables.tf` — Default/description for buildcache keep.
* `environments/production/terraform.tfvars` — `ecr_keep_last_n_buildcache = 0`.
...
* **`techx-corp-platform`:** None required. If CI still pushes `:buildcache`, those tags expire after ~1 day under this policy. Runtime tags (`sha-*`, `v*`) are unaffected.
...
| **Infrastructure** | ECR lifecycle policies updated on service repos after apply |
...
| **Cost** | Lower ECR storage over time (no long-lived buildcache digests) |
| **Backward compatibility** | Fully backward-compatible for deployable tags; buildcache retention reduced to age-based 1 day |
...
After apply, inspect a service repo:
...
2. Confirm lifecycle text as above; allow up to 24h for ECR to expire excess buildcache images.
3. Optional: list images and confirm buildcache tags disappear after the age window.
...
4. Lifecycle expiration is asynchronous; old buildcache digests may remain until ECR evaluates the policy.
...
2. Re-apply environments so rule 1 returns to `imageCountMoreThan` with count **1**.

<!-- Change trail: @hungxqt - 2026-07-22 - ECR lifecycle buildcache keep 0, images max 5. -->
